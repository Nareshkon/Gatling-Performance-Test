...
class ChromeBrowserTest extends Simulation{
  val httpProtocol = http
    ...
    .userAgentHeader("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.5790.110 Safari/537.36")
  ...
  val scn = scenario("Full User Journey")
    ...
    .exec(
      http("Delete a Computer")
        .post("/computers/6/delete")  // Delete the computer with ID 6 (this is an example, usually, you would delete by a dynamic ID)
        .check(status.is(200))
    )
    .pause(1.milliseconds)
